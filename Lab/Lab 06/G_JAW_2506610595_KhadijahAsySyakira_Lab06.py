...
def format_waktu_sesi(sesi):
    ...
    # TODO: lengkapi fungsi ini
    hari_index = [("Senin", HARI[0]), ("Selasa", HARI[1]), ("Rabu", HARI[2]), ("Kamis", HARI[3]), ("Jumat", HARI[4]), ("Sabtu", HARI[5]), ("Minggu", HARI[6])]
    court_names = []
    for elem in sesi:
        court_names.append(elem[0])

    for court in court_names:
        if court == sesi[0]:
            sesi[0] = str(court).capitalize
    
    day_start_value = sesi[1] // MENIT_DALAM_HARI
    day_end_value = sesi[2] // MENIT_DALAM_HARI
    sisa_menit_start = sesi[1] % MENIT_DALAM_HARI
    sisa_menit_end = sesi[1] % MENIT_DALAM_HARI

    sesi[1] = f"{konversi_hari(day_start_value)}, {sisa_menit_start//60}:{sisa_menit_start%60} s/d"
    sesi[2] = f"{konversi_hari(day_end_value)}, {sisa_menit_end//60}:{sisa_menit_end%60}"
    for court in court_names:
        if court == sesi[0]:
            sesi[0] = str(court).capitalize
...
def tambah_sesi():
    ...
    print(f"Sesi yang tersedia dan grup by coaching session: {format_by_coaching_session(KODE_SESI_TERSEDIA)}")
    ...
    for sesi in KODE_SESI_TERSEDIA:
        if sesi[0] == sesi_input:
            sesi_diambil.append(sesi)
        else:
            print("Sesi latihan tidak ditemukan")
...
def hapus_sesi():
    ...
    print(f"Sesi yang diambil dan group by coaching session: {format_by_coaching_session(sesi_diambil)}")
    ...
    for sesi in sesi_diambil:
        if sesi == sesi_input:
            sesi_diambil.remove(sesi_input)
    print()
...
def lihat_daftar_sesi():
    ...
    if not sesi_diambil :# TODO: cek apakah sesi_diambil kosong
        print("Belum ada sesi latihan yang diambil\n")
    else:
        print("daftar sesi yang telah diambil: ")
        # TODO: panggil fungsi format_by_coaching_session dengan parameter sesi_diambil
        print(f"{format_by_coaching_session(sesi_diambil)}")
